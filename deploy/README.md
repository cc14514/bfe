...
└── kubernetes/      # Kubernetes 部署
    ├── README.md                 # K8s 部署指南
    ├── conf-agent.toml.example   # conf-agent 配置示例
    ├── namespace.yaml            # 命名空间
...
```bash
# 推送镜像到仓库（如 ghcr.io）
docker tag bfe:$(cat VERSION) ghcr.io/your-org/bfe:$(cat VERSION)
docker push ghcr.io/your-org/bfe:$(cat VERSION)

# 修改 kubernetes/deployment.yaml 中的镜像地址后部署
...
- **默认行为**：容器启动时自动检测，conf-agent 不存在或未配置则跳过
- **BFE 配置**：通过 ConfigMap 挂载 `bfe.conf` 等配置文件到 `/home/work/bfe/conf/`（必需）
- **conf-agent 配置**：通过 ConfigMap 挂载 `conf-agent.toml` 到 `/home/work/conf-agent/conf/`（可选）
...
- **conf-agent 版本**：从 [docker/CONF_AGENT_VERSION](docker/CONF_AGENT_VERSION) 文件读取（如 `v0.0.2`）  - 版本号必须来自 [conf-agent 官方 releases](https://github.com/bfenetworks/conf-agent/releases/)
  - 只能使用已发布的版本号- **基础镜像标签**：使用 conf-agent 版本号
- **应用镜像标签**：使用 BFE 版本号
